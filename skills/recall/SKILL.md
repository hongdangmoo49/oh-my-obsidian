...
2. **Search Strategy**
   - **MCP Semantic Search** (optional): If the user has already configured an MCP server with
     recall/search capability, use it first. Do not install, start, or require an MCP server.
   - **Local Search** (default): Otherwise run the bundled keyword-search helper:
     ```
     node "$CLAUDE_PLUGIN_ROOT/plugins/oh-my-obsidian/scripts/vault-ops.mjs" recall --query "<query>"
     ```
     The helper walks local Markdown files and supplements those results with matching
     `session-catalog.json` metadata when available.
